...
**Post-M14 addendum (2026-08-18, same day, operator-requested): D-23 and
D-24 both closed/re-measured for real.** The Azure key was alive again
(D-20's sixth flip); the operator asked to spend the cost while it lasted.
`workflow/screening.py` now persists `<npi>.score.json` alongside every
`CasePacket` (the exact ~3-line fix D-23 always named), and a real, full
`scripts/40_screen.py --limit 250` re-run populated it:
`cohort_size=250 screened=240 rejected=10`. Found and fixed in the same
pass: `api/cases.py`'s cases-directory glob also matched the new
`*.score.json` files and crashed — caught by the existing suite,
fixed by excluding the suffix explicitly (2 tests failed, both green
after the fix). **The corpus now has 249 case packets** (245 before + 4
npis this run happened to accept that every earlier run had rejected —
LLM non-determinism, not a bug), **240 with an exact persisted score**, 9
still falling back to the approximate recompute (their most recent live
run rejected them, leaving a stale pre-fix packet with no score file —
handled correctly by the existing fallback, not an error state).

**The real, exact numbers now show 2 genuine `HIGH_PRIORITY` cases** —
`1003753369` and `1013394014`, both verified live (curl + browser
screenshot): 3 independent signal families each
(`network_anomaly`/`adverse_history`/`physical_existence`), citations
fully resolved, nothing manufactured. Full corpus: **2 HIGH_PRIORITY /
202 STANDARD / 45 LOW**. `cohort.html`'s D-24 caption, previously
hardcoded to "zero `high_priority` cases is a real, unaltered result," is
now conditional on the live count from `GET /cohort` — a static caption
that had gone stale the moment real data changed underneath it was caught
...
**`POST /screen`'s actual success path verified live minutes later — D-20's
8th flip, key alive again, operator asked to spend the cost while it
lasted.** Everything above only exercised the endpoint's dead-key failure
path; this is the real one. Picked `1033380456` — a real cohort provider
(`cohort_select(driver, "332", ["FL","TX","CA"], limit=260)`) that had
never been screened by any prior run — and triggered it through the actual
browser button, not curl: typed the NPI, clicked "Screen this provider,"
watched it show "Screening (real agent chain, ~30-90s)..." and disable
itself, and ~20 seconds later got "Screened 1033380456 — tier `low` — view
case." Server log confirms a real run: `graph_investigation` →
`enforcement_intel` → `skeptic` → `case_reporter`, real Azure calls
throughout, `POST /screen HTTP/1.1 200 OK`. Clicked through to the new
case page: real narrative (correctly reports zero fired signals and
declines to link two nearby real `EnforcementCase` hits to this provider,
both still `convicted` for whoever they actually belong to — hard rule 3
discipline holding under a brand-new live run, not just the historical
corpus), real network graph for a provider the dashboard had never seen
before (officer, phone, and address all rendered with real names), tier
badge marked **`(exact)`** — the D-23 persistence fix firing correctly on
its first true "new provider" case, not just the backfill re-run.
Confirmed on disk (`data/cases/1033380456.json` + `.score.json`, both
timestamped seconds after the click) and via `GET /cohort`: **250 total
cases now (249→250), 241 with an exact score (240→241).** `pytest tests/
-q` with the key alive: **335 passed**, all four previously Azure-dead
tests green again — no regression, same suite, just a live key this time.
...
| ~~D-10~~ | ~~`EnforcementCase.legal_status` graph node stays on the loader's keyword heuristic even after an agent adjudicates it~~ **CLEARED 2026-08-18, operator-requested.** `graph/enforcement_loader.update_legal_status_from_adjudications(driver, adjudications)` writes `CasePacket.legal_status_per_match` back onto the corresponding `EnforcementCase.legal_status` property; `workflow/screening.py`'s `screen_provider` calls it right after `case_reporter.synthesize` returns, inside the same `try` block (a `case_id` that doesn't resolve raises `SpecterError` — every `case_id` here already passed citation validation before the packet was assembled, so a miss is a real bug, not an expected outcome, and hard rule 7 means it fails loudly rather than silently). 3 new tests (`tests/test_enforcement_loader.py`), live round-trip against a real `EnforcementCase` node, restoring its original value afterward (Neo4j here is a shared, persistent instance, not a throwaway fixture). **Currently unexercised by the real 249-case corpus** — every case in it has zero `legal_status_per_match` entries, because `doj` (D-2) only has 1-2 real rows and none currently match a screened provider — so this write path is correct and tested but has never fired against production data yet. It will, the moment a real DOJ match exists. | — | **cleared** |
...
| ~~D-17~~ | ~~Synthetic scenario providers carry zero `HAS_TAXONOMY` edges, so `cohort_select` happens to never select them~~ **RESOLVED BY DESIGN, 2026-08-18, operator-requested — not by adding the edge.** Investigated adding `HAS_TAXONOMY` to `graph/loader_synthetic.py` (the literal reading of the debt) and rejected it: every synthetic scenario provider's raw data already carries `taxonomy_code: "332B00000X"` — DME, the exact prefix production screening filters on — and `cohort_select` had **no `data_origin` filter at all**. Adding the edge would have made synthetic providers eligible for a live `scripts/40_screen.py` cohort, silently mixing synthetic data into what M13/M14's dashboard reports as a "real" corpus (CLAUDE.md hard rule 5). Fixed the actual risk instead: `cohort_select` now filters `p.data_origin = 'public'` explicitly, so cohort selection is correct **by design**, not by the loader's accidental omission. New regression test confirms it (`test_cohort_select_excludes_synthetic_providers_explicitly`). Every consumer that needs a synthetic scenario already queries by `scenario_id` directly (M3/M5/M9's smoke scripts, `scripts/50_judge.py`) — the debt's own "route around it permanently" option, now made explicit and defended rather than left implicit. | — | **cleared** |
...
| ~~D-23~~ | ~~`CaseScore` is never written to disk~~ **CLEARED 2026-08-18, post-M14, operator-requested.** `workflow/screening.py`'s `screen_provider` now writes `<npi>.score.json` (`case_score.model_dump_json()`, verbatim) right alongside `<npi>.json` — the ~3-line edit the debt always named. `api/cases.py._score_for` prefers this persisted file when present, falling back to the same approximate recompute only for a case that predates the fix. Landed together with a real, live, full `scripts/40_screen.py --limit 250` re-run (Azure key came back alive — D-20's sixth state change — and the operator asked to spend the cost while it was live): `cohort_size=250 screened=240 rejected=10`, all 240 screened cases got a real `.score.json`. **Found and fixed in the same pass:** `api/cases.py._require_cases()`'s `CASES_DIR.glob("*.json")` also matched the new `*.score.json` files and crashed trying to parse one as a `CasePacket` — caught immediately by the existing test suite (2 failures), fixed by excluding the `.score.json` suffix explicitly. 249 case packets now exist on disk (245 before + 4 npis that were rejected in every earlier run but succeeded this time — real LLM non-determinism, not a bug); 9 of those 249 are orphaned (no `.score.json`, because *this* run happened to reject an npi a previous run had accepted) and still render `priority_tier_approximate: true` correctly via the fallback. | — | **cleared** |
| D-24 | **Re-measured 2026-08-18, post-M14, with real persisted scores — the "no provider can reach `HIGH_PRIORITY`" premise is no longer true.** The exact, persisted scores from the D-23 re-run show **2 real `HIGH_PRIORITY` cases** (`1003753369`, `1013394014`) — both fire `officer_degree`/`exclusion_proximity`/`physical_existence` (3 independent families: `network_anomaly`, `adverse_history`, `physical_existence`), both have `citation_report.all_resolved: true`, neither was manufactured or gated toward. Full live corpus (249 cases, 240 exact + 9 approximate): **2 HIGH_PRIORITY / 202 STANDARD / 45 LOW**. This is a genuinely different result from M13's live measurement three sessions ago (0/198/47, on a 245-case corpus using only *approximate* recompute) — not because the gate changed (`config/screening.yaml`'s `escalation_gate.min_independent_signal_families: 3` is untouched) but because this was a fresh live agent-chain run: `graph_investigation`/`case_reporter`'s evidence retrieval is non-deterministic run to run (M10's own documented finding), so two providers' *real* fired-signal sets differed enough this time to clear 3 families where no earlier run's evidence had. `cohort.html`'s D-24 caption is now conditional on the live count, not hardcoded to "zero" — verified live via browser screenshot. | Nothing is broken — the gate is plan §10's, verbatim, and it worked exactly as designed: most cases still don't clear it, and the 2 that do have real, resolved, three-family evidence. | **cleared as "zero", reopened as "the real live number, whatever it is."** Never hardcode 0, 2, or any other specific count into a template or doc claim again — `GET /cohort`'s `priority_tier_counts` is the only source of truth, and it will keep moving as the corpus is re-screened. |
