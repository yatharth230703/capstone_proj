...
class PromptCompiler:
    ...
    def _community_summaries_block(self) -> str:
        # B2: graph community summaries relevant to the cohort. The JSON body
        # is generated by graph/summaries.render_b2_community_summaries and
        # committed to prompts/blocks/b2_community_summaries.md by
        # scripts/30_build_communities.py — PromptCompiler has no Neo4j
        # driver and must stay constructible (and its output deterministic)
        # without one. The graph_version header is stamped here, not in the
        # generated file, so it always reflects *this* compiler instance.
        b2_path = self._blocks_dir / "b2_community_summaries.md"
        if not b2_path.exists():
            body = (
                "no community summaries generated yet — run "
                "scripts/30_build_communities.py --summaries"
            )
        else:
            body = b2_path.read_text().strip()
        return f"## B2: Community Context\n\n(graph_version={self._graph_version})\n\n{body}"
